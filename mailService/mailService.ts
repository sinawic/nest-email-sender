import { SupporterEmailsModels } from '../src/supporterEmails/supporterEmails.models'
...
const init = async () => {
  ...
  setInterval(async () => {
    ...
    const email = await SupporterEmailsModels.Email.aggregate([
      //@ts-ignore
      { $match: { sent: false } },
      { $sort: { 'date_created': 1 } },
      { $limit: 1 },
    ...
    if (email.length > 0) {
      ...
      try {
        ...
        await SupporterEmailsModels.Email.updateOne({ _id: email[0]._id }, { sent: true })
      } catch (err) {
        console.log(err)
      }
    } else
